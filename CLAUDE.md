...
One card on the appointment workspace's Check-In step. Each item is phrased as a plain question ("Overall symptom severity today?") with symptom-state labels ON the buttons ("No symptoms" … "Very severe" — labels describe the state, never the person, since the screen may be visible to the patient during a session; "Much better" … "Much worse" + "First visit / N/A"); the stored values remain the standard CGI 1–7 anchors, with full anchor text on hover. Speech dropdowns show plain labels ("Speaking pace: Rapid / hard to interrupt") but store the §24 constrained vocabulary — display language may change freely; stored values may not. One optional one-line note. Autosaves with the rest of the workspace; read-only once the appointment is completed. If it takes longer than 60 seconds, it has failed its design goal — resist adding fields.
